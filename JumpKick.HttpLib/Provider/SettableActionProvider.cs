...
namespace JumpKick.HttpLib.Provider
{
    ...
    {
        private Action<WebHeaderCollection, Stream, HttpWebResponse> success;
        private Action<WebException> fail;

        private NonActionProvider nonaction = new NonActionProvider();

        public SettableActionProvider(Action<WebHeaderCollection, Stream, HttpWebResponse> success, Action<WebException> fail)
        {
            if(success == null)
            {
